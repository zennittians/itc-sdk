...
import { isAddress, ChainType, hexToBN, ChainID, isValidAddress } from '@intelchain-js/utils';
import { getAddressFromPrivateKey, getAddress } from '@intelchain-js/crypto';
...
describe('test sign tranction', () => {
  ...
  it('should test sign transaction with Intelchain settings', () => {
    const itcMessenger = new Messenger(provider, ChainType.Intelchain, ChainID.Default);
    // tslint:disable-next-line: prefer-for-of
    for (let i = 0; i < reTxnVectors.length; i += 1) {
      ...
      transaction.setMessenger(itcMessenger);
  ...
  it('should test recover from IntelchainSignedTransaction', () => {
    const itcMessenger = new Messenger(provider, ChainType.Intelchain, ChainID.Default);
    // tslint:disable-next-line: prefer-for-of
    for (let i = 0; i < reTxnVectors.length; i += 1) {
      const vector = reTxnVectors[i];
      const factory = new TransactionFactory(itcMessenger);
      factory.setMessenger(itcMessenger);

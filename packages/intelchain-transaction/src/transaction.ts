...
import { add0xToString, numberToHex, ChainType, Unit } from '@intelchain-js/utils';
...
class Transaction extends TransactionBase {
  ...
  getRLPUnsigned(): [string, any[]] {
    ...
    const fields =
      this.messenger.chainType === ChainType.Intelchain ? transactionFields : transactionFieldsETH;
  ...
  getRLPSigned(raw: any[], signature: Signature): string {
    // temp setting to be compatible with eth
    const rawLength = this.messenger.chainType === ChainType.Intelchain ? 11 : 9;
  ...
  recover(rawTransaction: string): Transaction {
    // temp setting to be compatible with eth
    const recovered =
      this.messenger.chainType === ChainType.Intelchain
   ...
   *   shardID: 1,
   *   toShardID: 0,
   *   gasPrice: new itc.utils.Unit('101').asGwei().toWei(),
   *   signature: {

...
import { BN, getAddress, IntelchainAddress } from '@intelchain-js/crypto';
import { hexToNumber } from '@intelchain-js/utils';
import { Messenger, RPCMethod, Emitter, HttpProvider, NewHeaders } from '@intelchain-js/network';
...
export class TransactionBase implements AbstractTransaction {
  static normalizeAddress(address: string) {
    if (address === '0x') {
      ...
      IntelchainAddress.isValidChecksum(address) ||
      IntelchainAddress.isValidBech32(address) ||
      IntelchainAddress.isValidBech32TestNet(address)
    ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      newHeads.then((p) => {
        p.onData(async (data: any) => {
          const blockNumber =
            this.messenger.chainPrefix === 'itc'

...
import { Transaction, TransactionFactory } from '@intelchain-js/transaction';
import { Blockchain } from './blockchain';
import { ContractFactory } from '@intelchain-js/contract';
...
export class IntelchainExtension {
  /**@ignore*/
   ...
   *
   * @param wallet could be MathWallet or ItcWallet instance
   ...
   *   itcEx = new IntelchainExtension(window.intelchain);
   * }
   * // Using ItcWallet instance
   * export const initEx = async() => {
   *   itcEx = new IntelchainExtension(window.itcwallet);
   * }
   * ```
   */
  constructor(
    ...
    this.wallet = wallet;
    // check if it is mathwallet or itcwallet
   ...
   * @example
   * ```javascript
   * const tmp = itcEx.setProvider('http://localhost:9500');
   * ```
   */
  ...
  public isExtension(wallet: ExtensionInterface) {
    ...
    this.extensionType = null;
    if (wallet.isMathWallet || wallet.isItcWallet) {
      isExtension = true;
      if (wallet.isMathWallet) this.extensionType = ExtensionType.MathWallet;
      else this.extensionType = ExtensionType.ItcWallet;

      // remake signTransaction of MathWallet or ItcWallet

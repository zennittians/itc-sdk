...
Create a Intelchain instance connecting to testnet
...
*   Units,
*   Unit,
* } = require('@intelchain-js/utils');

* const itc = new Intelchain(
*     'https://api.s0.b.intelchain.org/',
*     {
*         chainType: ChainType.Intelchain,
*         chainId: ChainID.ItcTestnet,
*     },
* );
```
...
*   .getBalance({ address: '' })
*   .then((response) => {
*     console.log('balance in ITCs: ' + fromWei(hexToNumber(response.result), Units.itc));
*   });
```

Getting the latest block number
```javascript
* itc.blockchain.getBlockNumber().then((response) => {
...
```javascript
* itc.blockchain.getShardingStructure().then((response) => {
*   console.log(response.result);
* });
```
...
```javascript
// key corresponds to , only has testnet balance
* itc.wallet.addByPrivateKey('45e497bd45a9049bcb649016594489ac67b9f052a6cdf5cb74ee2427a60bf25e');

* async function transfer() {
*   const txn = itc.transactions.newTx({
*     to: '',
*     value: new Unit(1).asItc().toWei(),
...
*     gasPrice: new itc.utils.Unit('1').asGwei().toWei(),
*   });

*   // sign the transaction use wallet;
*   const signedTxn = await itc.wallet.signTransaction(txn);
*   const txnHash = await itc.blockchain.sendTransaction(signedTxn);
...
import {
  ...
  // IntelchainCore,
  DefaultBlockParams,
} from '@intelchain-js/utils';

import { getAddress } from '@intelchain-js/crypto';

import { Transaction } from '@intelchain-js/transaction';
import { StakingTransaction } from '@intelchain-js/staking';
...
class Blockchain {
   ...
   * @example
   * ```javascript
   * itc.blockchain.getBlockNumber().then((value) => {
   *   console.log(value.result);
   * });
   * ```
   */
   ...
   * @example
   * ```javascript
   * itc.blockchain.getBlockByNumber({
   *   blockNumber: '0x89',
   * }).then((value) => {
   *   console.log(value);
   * });
   ...
   * @example
   * ```javascript
   * itc.blockchain.getBlockTransactionCountByHash({
   ...
   * @example
   * ```javascript
   * itc.blockchain.getBlockTransactionCountByNumber({
   *   blockNumber: '0x2403C'
   * }).then((value) => {
   *   console.log(value);
   * });
   ...
   * @example
   * ```javascript
   * itc.blockchain.getTransactionByBlockHashAndIndex({
   *   blockHash: '0x4142514a238157e7fe57b9d54abedb33943507fa15b3799954c273a12705ced1',
   *   index: '0x0'
   * }).then((value) => {
   *   console.log(value);
   ...
   * @example
   * ```javascript
   * itc.blockchain.getTransactionByBlockNumberAndIndex({
   *   blockNumber: '0x2403C',
   *   index: '0x0'
   * }).then((value) => {
   *   console.log(value);
   ...
   * @example
   * ```javascript
   * itc.blockchain.getTransactionByHash({
   ...
   * @example
   * ```javascript
   * itc.blockchain.getTransactionReceipt({
   *   txnHash: '0x146a0cf7e8da45b44194207c4e7785564527059483b765f9a04424554443b224'
   * }).then((value) => {
   *   console.log(value);
   * });
   ...
   * itc.blockchain.getCxReceiptByHash({
   ...
   * @example
   * ```javascript
   * itc.blockchain.net_peerCount().then((value) => {
   *   console.log(value.result);
   * });
   * ```
   */
   ...
   * @example
   * ```javascript
   * itc.blockchain.net_version().then((value) => {
   *   console.log(value.result);
   * });
   * ```
   */
   ...
   * @example
   * ```javascript
   * itc.blockchain.getProtocolVersion().then((value) => {
   *   console.log(value.result);
   * });
   * ```
   */
   ...
   * @example
   * ```javascript
   * itc.blockchain.getTransactionCount({
   *   address: ""
   * }).then((value) => {
   *   console.log(value.result);
   * });
   ...
   * @example
   * ```javascript
   * itc.blockchain.getShardingStructure().then((value) => {
   *   console.log(value);
   * });
   * ```
   */
   ...
   * itc.wallet.addByPrivateKey(privateKey);
   *
   * async function transfer() {
   *   const txn = itc.transactions.newTx({
   *     //  token send to
   ...
   *     toShardID: 0,
   *     // gas Price, you can use Unit class, and use Gwei, then remember to use toWei(), which will be transformed to BN
   *     gasPrice: new itc.utils.Unit('100').asGwei().toWei(),
   *   });
   *
   *   // sign the transaction use wallet;
   *   const signedTxn = await itc.wallet.signTransaction(txn);
   ...
   *
   *   const txnHash = await itc.blockchain.sendRawTransaction(recovered);
   *   console.log(txnHash);
   * }
   *
   * transfer();
   ...
   *   // sign the transaction use wallet;
   *   const signedTxn = await itc.wallet.signTransaction(txn);
   *   const txnHash = await itc.blockchain.createObservedTransaction(signedTxn);
   *   console.log(txnHash);
   * }
   *
   * transfer();
   ...
   * @warning
   * ```
   * At present, this function itc_estimateGas is not implement yet, will Coming soon!!!
   * ```
   *
   * @example
   * ```javascript
   * itc.blockchain.estimateGas({
   *   to: '',
   ...
   * @example
   * ```javascript
   * itc.blockchain.gasPrice().then((value) => {
   *   console.log(value);
   * });
   * ```
   */
   ...
   * );
   *
   * const tmp = itc.blockchain.newPendingTransactions();
   * console.log(tmp)
   * ```
   */
   ...
   * );
   *
   * const tmp = itc.blockchain.newBlockHeaders();
   * console.log(tmp)
   * ```
   */
   ...
   * );
   *
   * const tmp = itc.blockchain.syncing();
   * console.log(tmp)
   * ```
   */

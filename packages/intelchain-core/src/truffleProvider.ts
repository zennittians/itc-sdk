...
import { ChainID, ChainType, Unit, isBech32Address } from '@intelchain-js/utils';
import { HDNode } from '@intelchain-js/account';
import { fromBech32, HRP } from '@intelchain-js/crypto';
...
export class TruffleProvider extends HDNode {
  ...
  async send(...args: [RPCRequestPayload<any>, any]) {
    ...
    switch (newMethod) {
      ...
      case 'itc_getTransactionReceipt': {
        const result = await this.provider.send(
          {
            id,
            method: 'itc_getTransactionReceipt',
            params: [params[0]],
            jsonrpc: '2.0',
          },
          (err: any, res: any) => {
      ...
      case 'web3_clientVersion': {
        callback(null, {
          result: `Intelchain/${packageInfo.version}/@intelchain-js`,
          id,
          jsonrpc: '2.0',
        });
        return {
  ...
  resolveArgs(...args: [RPCRequestPayload<any>, any]): ArgsResolver {
    ...
    if (method.startsWith('eth')) {
      newMethod = method.replace('eth', 'itc');
    }

...
import { getAddress, IntelchainAddress } from '../src/address';
...
describe('test address', () => {
  it('should test HamonyAddress', () => {
    const hAddress = new IntelchainAddress(bytes);
    ...
    expect(IntelchainAddress.isValidBasic(bytes)).toEqual(true);
    expect(IntelchainAddress.isValidChecksum(checksum)).toEqual(true);
    expect(IntelchainAddress.isValidBech32(bech32)).toEqual(true);
    expect(IntelchainAddress.isValidBech32TestNet(bech32Testnet)).toEqual(true);

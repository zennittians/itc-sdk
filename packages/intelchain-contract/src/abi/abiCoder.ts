...
```javascript
const { ContractFactory } = require('@intelchain-js/contract');
const { Wallet } = require('@intelchain-js/account');
const { Messenger, HttpProvider } = require('@intelchain-js/network');
const { ChainID, ChainType, hexToNumber } = require('@intelchain-js/utils');
...
* const itc = new Intelchain("https://api.s0.b.intelchain.org", {
...
const contract = itc.contracts.createContract(contractJson.abi, contractAddr);
...
Call contract read-only methods. Intelchain uses 1 Gwei gas price and gas limit of 21000 by default. Use the estimate gas api to correctly set the gas limit.
```javascript
* const options1 = { gasPrice: '0x3B9ACA00' }; // gas price in hex corresponds to 1 Gwei or 1000000000
* let options2 = { gasPrice: 1000000000, gasLimit: 21000 }; // setting the default gas limit, but changing later based on estimate gas
...
* const { ContractFactory } = require('@intelchain-js/contract');
* const { Wallet } = require('@intelchain-js/account');
* const { Messenger, WSProvider } = require('@intelchain-js/network');
* const { ChainID, ChainType, hexToNumber } = require('@intelchain-js/utils');
* const ws = new WSProvider('wss://ws.s0.b.intelchain.org');
...
import {
  ...
  checkNew,
  bytesPadRight,
} from '@intelchain-js/crypto';
import { hexToBN, defineReadOnly } from '@intelchain-js/utils';

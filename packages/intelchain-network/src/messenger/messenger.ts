...
import { IntelchainCore, ChainType, isString, ChainID, defaultConfig } from '@intelchain-js/utils';
 ...
 * ```
 */
class Messenger extends IntelchainCore {
  ...
  async setShardingProviders() {
    if (this.chainPrefix !== ChainType.Intelchain) {
      return;
    }
    try {
   ...
   * @example
   * ```
   * itc.messenger.getShardProvider()
   * ```
   */
   ...
   * @example
   * ```
   * itc.messenger.getCurrentShardID()
   * ```
   */

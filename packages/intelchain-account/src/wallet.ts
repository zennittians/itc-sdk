...
import { bip39, hdkey, EncryptOptions, getAddress, generatePrivateKey } from '@intelchain-js/crypto';
import { Messenger } from '@intelchain-js/network';
import { isPrivateKey, isAddress, ChainType } from '@intelchain-js/utils';
...
class Wallet {
   ...
   * @example
   * ```
   * const { Wallet } = require('@intelchain-js/account');
   * const { HttpProvider, Messenger } = require('@intelchain-js/network');
   * const { ChainType, ChainID } = require('@intelchain-js/utils');
   *
   * // create a custom messenger
   * const customMessenger = new Messenger(
   *   new HttpProvider('http://localhost:9500'),
   *   ChainType.Intelchain, // if you are connected to Intelchain's blockchain
   *   ChainID.ItcLocal, // check if the chainId is correct
   * )
   *
   * const wallet = new Wallet(customMessenger);
   * ```
  ...
  addByMnemonic(phrase: string, index: number = 0) {
    ...
    // TODO:hdkey should apply to Intelchain's settings
    const path = this.messenger.chainType === ChainType.Intelchain ? '1023' : '60';
   ...
   * const key_1 = '45e497bd45a9049bcb649016594489ac67b9f052a6cdf5cb74ee2427a60bf25e';
   * wallet.addByPrivateKey(key_1);
   * wallet.encryptAccount('', '12345').then((value) => {
   *   console.log(value);
   * })
   * ```
   */
   ...
   * const key_1 = '45e497bd45a9049bcb649016594489ac67b9f052a6cdf5cb74ee2427a60bf25e';
   * wallet.addByPrivateKey(key_1);
   * wallet.encryptAccount('', '12345')
   * .then(() => {
   *   wallet.decryptAccount('', '12345')
   *   .then((value) =>{
   *      console.log(value);
   *   });
   * });
   ...
   * ```javascript
   * const customMessenger = new Messenger(
   *   new HttpProvider('https://api.s0.b.intelchain.org'),
   *   ChainType.Intelchain, // if you are connected to Intelchain's blockchain
   *   ChainID.ItcLocal, // check if the chainId is correct
   * )

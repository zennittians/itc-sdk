...
import { bip39, hdkey, getAddress, BN, Signature } from '@intelchain-js/crypto';
import {
  HDPath,
  // defineReadOnly,
  isHttp,
...
import { Messenger, HttpProvider, WSProvider, RPCMethod } from '@intelchain-js/network';
import { Transaction, TxStatus, TransasctionReceipt } from '@intelchain-js/transaction';
...
export class HDNode {
  ...
  constructor(
    ...
    shardID: number = 0,
    chainType: ChainType = ChainType.Intelchain,
    ...
    this.addresses = [];
    this.wallets = {};
    this.path = chainType === ChainType.Intelchain ? HDPath : `m/44'/60'/0'/0/`;
    this.index = index;

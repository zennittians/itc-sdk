...
export const unitMap = new Map([
  ...
  [Units.itc, '1000000000000000000'], // 1e18 wei
...
/**
 * Converts any ITC value into wei
 */
...
/**
 * Converts any wei value into a ITC value.
 */
...
export class Unit {
  ...
  static Itc(str: BN | string) {
    return new Unit(str).asItc();
  }
  ...
  asItc() {
    this.wei = toWei(this.unit, Units.itc);
    return this;
  }
  asKether() {
  ...
  toItc() {
    if (this.wei) {
      return fromWei(this.wei, Units.itc);
    } else {
      throw new Error('error transforming');
    }
  }

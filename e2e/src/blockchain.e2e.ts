...
const bc = intelchain.blockchain;
...
describe('e2e test blockchain', () => {
  ...
  it('should test net_peerCount', async () => {
    ...
    expect(intelchain.utils.isHex(peerCount.result)).toEqual(true);
  });
  ...
  it('should test itc_getBlockByNumber', async () => {
    ...
    expect(res.responseType).toEqual('raw');
    expect(intelchain.utils.isHex(size)).toEqual(true);
    ...
    expect(intelchain.utils.isHex(res2.result.size)).toEqual(true);
  ...
  it('should test itc_getBlockByHash', async () => {
    fetch.mockResponse(
      JSON.stringify({
        "jsonrpc": "2.0",
        "id": 1,
    ...
    expect(intelchain.utils.isHex(res.result.size)).toEqual(true);
    expect(checkBlockData(res.result)).toEqual(true);
  ...
  // account related
  it('should test itc_getBalance', async () => {
    fetch.mockResponseOnce(
      JSON.stringify({"jsonrpc": "2.0", "id": 1, "result": "0x10"}),
    );
    const balance = await bc.getBalance({ address: testAccount.Address });
    expect(intelchain.utils.isHex(balance.result)).toEqual(true);
...
function checkBlockData(data: any) {
  return intelchain.utils.validateArgs(
    data,
    {
      difficulty: [intelchain.utils.isNumber],
      // tslint:disable-next-line: no-shadowed-variable
      extraData: [(data: any) => data === '0x' || intelchain.utils.isHex(data)],
      gasLimit: [intelchain.utils.isHex],
      gasUsed: [intelchain.utils.isHex],
      hash: [intelchain.utils.isHash],
      logsBloom: [intelchain.utils.isHex],
      miner: [intelchain.utils.isBech32Address],
      mixHash: [intelchain.utils.isHash],
      nonce: [intelchain.utils.isNumber],
      number: [intelchain.utils.isHex],
      parentHash: [intelchain.utils.isHash],
      receiptsRoot: [intelchain.utils.isHash],
      size: [intelchain.utils.isHex],
      stateRoot: [intelchain.utils.isHash],
      timestamp: [intelchain.utils.isHex],
      transactionsRoot: [intelchain.utils.isHash],
      uncles: [intelchain.utils.isArray],
    },
    { transactions: [intelchain.utils.isArray] },
  );
}